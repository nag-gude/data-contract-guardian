...
from app.services.fivetran_mcp_stdio import build_tool_arguments, call_fivetran_mcp_tool_stdio, mcp_stdio_available
...
def fivetran_mcp_proxy(body: dict[str, Any] = Body(default_factory=dict)):
    """
    Read-only proxy for a single Fivetran MCP tool call.

    Used by the ADK Agent Engine playground when native stdio MCP is disabled.
    Spawns the real fivetran-mcp server on Cloud Run (isolated from the Engine venv).
    """
    tool = str(body.get("tool") or "").strip()
    if not tool:
        raise HTTPException(400, detail="tool is required")
    allowed = set(MCP_DISCOVERY_TOOLS) | {
        "get_connection_schema_config",
    }
    if tool not in allowed:
        raise HTTPException(400, detail=f"tool not allowed: {tool}")

    if not mcp_stdio_available():
        return {
            "capability": "fivetran_mcp_proxy",
            "disclaimer": AGENT_DISCLAIMER,
            "tool": tool,
            "ok": False,
            "error": "Fivetran MCP not configured (mock mode or missing credentials)",
        }

    arguments = dict(body.get("arguments") or {})
    connection_id = arguments.get("connection_id") or body.get("connection_id")
    try:
        if "schema_file" not in arguments:
            arguments = build_tool_arguments(tool, connection_id=str(connection_id) if connection_id else None)
        elif connection_id and "connection_id" not in arguments:
            arguments["connection_id"] = connection_id
        payload = call_fivetran_mcp_tool_stdio(tool, arguments)
        return {
            "capability": "fivetran_mcp_proxy",
            "disclaimer": AGENT_DISCLAIMER,
            "tool": tool,
